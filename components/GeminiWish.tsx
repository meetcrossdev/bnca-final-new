...
interface GeminiWishProps {
  timezone: string; // kept for parent compatibility
}

export const GeminiWish = (_props: GeminiWishProps) => {
  ...
  const verseText =
    separatorIndex > -1
      ? verseData.substring(0, separatorIndex).replace(/^"|"$/g, '')
      : verseData;
  const verseRef =
    separatorIndex > -1 ? verseData.substring(separatorIndex + 3) : '';
  ...
  return (
    <div className="max-w-2xl w-full mx-auto mt-12 p-8 rounded-2xl bg-slate-900/40 backdrop-blur-md border border-slate-700/50 text-center transition-all duration-700 hover:bg-slate-800/50 hover:border-slate-600 group min-h-[180px] flex flex-col justify-center relative overflow-hidden">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-1 bg-gradient-to-r from-transparent via-gold-500/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center animate-in fade-in slide-in-from-bottom-2 duration-1000">
          ...
          <div className="absolute bottom-0 left-0 h-0.5 bg-slate-700/30 w-full">
            <div
              className="h-full bg-gold-500/30 animate-[width_60s_linear_infinite]"
              style={{ width: '0%' }}
            />
          </div>
        </div>
