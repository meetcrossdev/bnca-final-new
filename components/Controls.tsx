  ...
  return (
    <div className="flex flex-row items-center gap-2 p-2 bg-slate-900/80 backdrop-blur-xl border border-slate-700/50 rounded-2xl shadow-2xl ring-1 ring-white/10">
      ...
      <div className="flex items-center gap-1 sm:gap-2 px-1">
        ...
        {/* Volume Slider */}
        <div className="hidden sm:flex items-center w-16 md:w-24 px-1">
          <input
            type="range"
            min="0"
            max="1"
      ...
      >
        <span className="relative flex h-3 w-3 sm:hidden">
          ...
          <Sparkles
            className={`w-3 h-3 ${
              isPreviewMode ? 'text-gold-400' : 'text-slate-400'
            }`}
          />
        </span>
        ...
        <Sparkles
          className={`hidden sm:block w-4 h-4 ${
            isPreviewMode ? 'animate-pulse' : ''
          }`}
        />

        <span className="hidden sm:inline">
          {isPreviewMode ? 'Stop' : 'Test'}
        </span>
      </button>
